...
public class Car extends Vehicle {
	...
	public void refuel(int amountOfGas) {
		...
		this.gasRemained = this.gasRemained + amountOfGas;
	...
	public void run(int hour) {
		...
		{
		    total = this.gasConsumedPerHour * hour; 
			this.gasRemained = this.gasRemained - total;
			if(this.gasRemained<=0)
			{
				System.out.println("Oops, gas out! Please add fuel!");
				val = total - Math.abs(this.gasRemained);
				this.totalGasConsumed += val;
				this.gasRemained = 0;
			}
			else
			{
				this.totalGasConsumed += total;
			}
		}
		return;
	}
